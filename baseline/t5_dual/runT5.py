...
from T5TwoTower import T5TwoTower
...
parser.add_argument("--only_encoder", default=False, type=bool, help="only encoder / encoder decoder.")
...
def train_model(train_data, test_data):
    pretrain_model = T5EncoderModel.from_pretrained(args.pretrain_model_path)
    model = T5TwoTower(pretrain_model, args)
...
def inference(test_data):
    ...
    with open(args.infer_output, "w") as fw:
        for k, v in id_to_embed.items():
            v = [str(i) for i in v.numpy().tolist()]
            fw.write(k+'\t'+','.join(v)+'\n')
