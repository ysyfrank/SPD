...
parser.add_argument("--pretrain_model_path", default="/home/xxx/transformers_models/bert/", type=str, help='bert model path')
...
def add_docid_to_vocab(doc_file_path):
    ...
    with open(doc_file_path) as fin:
        for i, line in tqdm(enumerate(fin), desc='constructing all_documents list'):
            ...
            all_documents += [("[{}]".format(docid),sents)]
            if args.add_doc_id:
                new_docid_token = "[{}]".format(docid)
                new_tokens += [new_docid_token]
...
def gen_train_instance(id_to_token, token_to_id, all_docid):
    ...
    with open(args.data_path) as fin:
        for i, line in tqdm(enumerate(fin), desc='constructing all_documents list'):
            ...
            # url = data['title'].lower()
            body = data['body'].lower()
            docid = data['docid'].lower()
            docid = "[{}]".format(docid)

            did_2_doc[docid] = title + ' ' + body
