...
def initialize_docid_embed(bert_model_path, doc_file_path, bert_embedding, docs_embed):
    """
        args:
    ...
    with open(doc_file_path) as fin:
        for i, line in tqdm(enumerate(fin), desc='constructing all_documents list'):
            ...
            body = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(data["body"]))
            token_count = Counter(body)
            embed = docs_embed[i]
            for token_id, count in token_count.items():
                embed += bert_embedding[token_id]
            embed /= (len(token_count)+1)
            docs_embed[i] = embed
            doc_tokens[start_doc_id+i] = (len(body), token_count) # body的总长，以及各个token的数量
    with open("./docs_embed_init.pkl", "wb") as fw:
