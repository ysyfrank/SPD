...
class T5ForPretrain(T5ForConditionalGeneration):
    def __init__(self, t5_model, args):
        ...
        if self.args.load_gtr == "True": # whether to load gtr as the encoder
            print("load gtr dense layer.")
            self.dense = nn.Linear(self.config.d_model, self.config.d_model, bias=False)
            if "t5-base" in args.pretrain_model_path: 
                weight_path = os.path.join(f"/mnt/xxx/transformers_models/gtr-{os.path.split(args.pretrain_model_path)[1]}/linear.weight")
                self.dense.weight = nn.Parameter(pickle.load(open(weight_path, "rb")))
    ...
    def forward(self,
        ...
        if self.args.load_gtr == "True":
            hidden_states = hidden_states * attention_mask.unsqueeze(2)  # mask embedding of pad tokens
            hidden_states = torch.sum(hidden_states, axis=1) / torch.sum(attention_mask, axis=1).unsqueeze(1) # [bs, hidden_dim]
            hidden_states = self.dense(hidden_states).unsqueeze(1) # [bs, hidden_dim]
